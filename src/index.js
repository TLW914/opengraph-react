...
export default class OpengraphReactComponent extends Component {
  ...
  getResultsToUse = () => {
    ...
    const site_name =
      openGraph.site_name || htmlInferred.site_name || hybridGraph.site_name;
        ...
        <p>
          {!this.props.hidePrice && (
            <span className={"priceText"}>{`${
              !!currencyMatch ? currencyMatch.symbol : ""
            }${price}`}</span>
          )}
          <span
            className={"greyText"}
          >{` from ${resultsToUse.site_name}`}</span>
        </p>
      );
    }
  };
    ...
    return (
      <div className='outerWrapperSmall'>
        ...
        <div className={"textWrapperSmall"}>
          ...
          {!!goodProduct.totalRating &&
            this.renderStarsForRating(goodProduct.totalRating)}
  ...
  renderLarge = resultsToUse => {
    let imageClassName = this.props.spin
      ? "responsiveImage App-logo"
      : "responsiveImage";
